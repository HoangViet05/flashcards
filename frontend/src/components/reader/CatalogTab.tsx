...
export default function CatalogTab({ onAdopted }: { onAdopted: () => void }) {
  ...
  useEffect(() => { let active = true; setItems(null); getCatalog(level).then(data => { if (active) setItems(data) }).catch(() => { if (active) { setItems([]); toast('The catalog could not be loaded.', 'error') } }); return () => { active = false } }, [level, toast])
  ...
  return <><div className="mb-4 flex flex-wrap gap-2">{LEVELS.map(value => <button key={value} onClick={() => chooseLevel(value)} className={`rounded-xl border px-3 py-2 text-sm font-bold transition ${value === level ? 'border-cyan-300/30 bg-cyan-400/15 text-cyan-100' : 'border-white/10 bg-white/[.04] text-slate-400 hover:bg-white/[.07]'}`}>{READING_LEVEL_LABELS[value]}</button>)}</div>{items === null ? <div className="grid gap-3 sm:grid-cols-2">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl bg-white/[.05]" />)}</div> : items.length === 0 ? <p className="rounded-2xl border border-white/[.07] bg-white/[.03] p-6 text-center text-sm text-slate-400">There are no readings at this level yet. Import the catalog to add one.</p> : <div className="grid gap-3 sm:grid-cols-2">{items.map(item => <button key={item.id} onClick={() => setPreview(item.id)} className="rounded-2xl border border-white/[.07] bg-white/[.03] p-4 text-left transition hover:border-cyan-300/20"><h3 className="line-clamp-2 font-bold text-slate-100">{item.title}</h3><p className="mt-2 text-xs text-slate-500">{SOURCE_LABELS[item.source] ?? item.source} · {item.word_count} words · {item.suggested_word_count} suggested</p>{item.already_added && <span className="mt-3 inline-flex rounded-full border border-emerald-300/20 bg-emerald-300/10 px-2 py-1 text-[11px] font-bold text-emerald-200">In your Reader</span>}</button>)}</div>}{preview && <CatalogPreview catalogId={preview} onClose={() => setPreview(null)} onAdopted={reload} />}</>
}
