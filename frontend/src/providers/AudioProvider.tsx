...
type AudioState = { silent: boolean; toggleSilent: () => void; feedback: (kind: 'correct' | 'wrong' | 'combo' | 'complete' | 'checkpoint') => void; ui: () => void; playAmbient: (kind: 'focus' | 'reader' | 'boss') => void; stopAmbient: () => void; duckAmbient: (active: boolean) => void }
...
export function AudioProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => ({ silent, toggleSilent, feedback, ui, playAmbient, stopAmbient, duckAmbient }), [silent, toggleSilent, feedback, ui, playAmbient, stopAmbient, duckAmbient])
