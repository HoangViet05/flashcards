...
export default function VoiceStage({ state, source, target, children }: { state: VoiceState; source: string; target: string; children: ReactNode }) {
  const status = {
    ready: 'Listen, then record your response.',
    recording: 'Recording your voice…',
    processing: 'Scoring your signal…',
    score: 'Signal scored. Choose the next move.',
    offline: 'Offline practice keeps your recording private.',
  }[state]
