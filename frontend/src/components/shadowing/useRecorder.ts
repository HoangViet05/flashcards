...
export function useRecorder(maxSeconds = 20) {
  ...
  useEffect(() => {
    if (recording) document.body.dataset.shadowingListening = 'true'
    else delete document.body.dataset.shadowingListening
    return () => { delete document.body.dataset.shadowingListening }
  }, [recording])
