...
export function AiOrb({ state = 'idle', compact = false, label, showMessage = true, onActivate, activationLabel = 'Show learning status' }: AiOrbProps) {
  ...
  const visual = <><span className="ai-orb__orbit" aria-hidden="true" /><span className="ai-orb__orbit ai-orb__orbit--outer" aria-hidden="true" /><span className="ai-orb__core" aria-hidden="true" />{['listening', 'recording', 'processing'].includes(state) && <span className="ai-orb__signal" aria-hidden="true">{[32, 72, 100, 56, 84].map((height, index) => <i key={index} style={{ '--h': `${height}%` } as React.CSSProperties} />)}</span>}</>
